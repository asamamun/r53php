...
if (isset($_POST['addBtn'])) {
    ...
    $insertQ = "insert into categories values(null,?,?,?,current_timestamp())";
    // Create a statement object
    $stmt = $connect->stmt_init();
    // Prepare the statement for execution
    $stmt->prepare($insertQ);
    // Bind the parameters
    $stmt->bind_param('sss', $name, $desc, $icon);
    $stmt->execute();
    // Recuperate the statement resources
    $stmt->close();
    //reload the page so that in case of refresh form does not submit again
    ...
    <div class="container">
        ...
        <div id="tableContainer">
            <table class="table">
                ...
                $stmt = $connect->stmt_init();
                // Prepare the statement for execution
                $stmt->prepare($selectQ);
                // Execute the statement
                $stmt->execute();
                // Bind the result parameters
                $stmt->bind_result($id, $name, $d, $icon, $time);
                ...
                while ($stmt->fetch()) {
                    ...
                    echo "<td>" . $name . "</td>";
                    echo "<td>" . $d . "</td>";
                    echo "<td><img class='iconimage' src='assets/images/caticon/" . $icon . "' /></td>";
                    echo "<td>" . $time . "</td>";
                    echo "<td><a href='categories-edit.php?id=" . $id . "'><i class='bi bi-pencil-square'></i></a> | <a onclick='return confirm(\"Are you sure you want to delete this item?\");' href='categories-delete.php?id=" . $id . "'><i class='bi bi-trash'></i></a></td>";
                    echo "</tr>";
                }
                // Recuperate the statement resources
                $stmt->close();
